...
HEAD_TRAJECTORY_DT = 0.012    # 表示头部轨迹相邻两帧之间的时间；数值越大，摇头越慢
    ...
    try:
        ...
        if not shutdown_event.is_set():
            if emotion is not None:
                emotion.stop()
            # 曲终不再受节奏限制。必须等待每只手臂完整回到原位，不能像演奏中的
            # 换键一样只等待 HAND_MOVE_DELAY 后就开始手指复位/机器人关机。
            # 否则未完成的关节运动会被后续指令打断，表现为回位时突然一卡。
            restored_any_arm = False
            if left_hand_moved:
                if move_hand_to(
                    motion,
                    robot,
                    list(left_origin_pose),
                    G1JointGroup.left_arm,
                    is_blocking=True,
                ):
                    current_left_stop = "BASE"
                    restored_any_arm = True
            if right_hand_moved:
                if move_hand_to(
                    motion,
                    robot,
                    list(right_origin_pose),
                    G1JointGroup.right_arm,
                    is_blocking=True,
                ):
                    current_right_stop = "BASE"
